...
/** @var array $V */
/** @var string $XML_DATA */
use Bitrix\Currency, Bitrix\Iblock, Bitrix\Catalog;
...
//	'place', 'hall', 'hall_part', 'is_premiere', 'is_kids', 'date'
    ...
    'title', 'description', 'param', 'g:product_type', 'g:google_product_category'
);
...
//    fwrite($fp, '<title>'. $APPLICATION->ConvertCharset(htmlspecialcharsbx(COption::GetOptionString('main', 'site_name', '')), LANG_CHARSET, 'utf-8') ."</title>\n");
    fwrite($fp, '<title>«Кулерком» - интернет-магазин кулеров для воды и оборудования к ним</title>'."\n");
    fwrite($fp, "<link>".$usedProtocol.htmlspecialcharsbx($ar_iblock['SERVER_NAME'])."</link>\n");
    fwrite($fp, "<description>Интернет-магазин «Кулерком» - продажа кулеров, пурифайеров, помп для воды и аксессуары</description>\n"); 
...
//                            continue;
                        ...
                        $title_text = gmcConvertText(yandex_text2xml($arAcc["~NAME"], true));
...
/*
    else if ($arCatalog['CATALOG_TYPE'] == CCatalogSku::TYPE_PRODUCT || $arCatalog['CATALOG_TYPE'] == CCatalogSku::TYPE_FULL)
    {
        $arOfferSelect = array(
            "ID", "LID", "IBLOCK_ID", "NAME",
            "PREVIEW_PICTURE", "PREVIEW_TEXT", "PREVIEW_TEXT_TYPE", "DETAIL_PICTURE", "DETAIL_PAGE_URL",
            "CATALOG_AVAILABLE", "CATALOG_TYPE", "DETAIL_TEXT", "DETAIL_TEXT_TYPE", "LIST_PAGE_URL", "SECTION_PAGE_URL", "CODE"
        );
...
//                exit();
                ...
                {
                    $strSections.= "<g:adwords_grouping>".$arSection["NAME"]."</g:adwords_grouping>\n";
                    $boolNoActiveSections = false;
                }
            }
            if (!$boolCurrentSections) {
                $boolNeedRootSection = true;
                $strSections.= "<g:adwords_grouping>".$intMaxSectionID . "</g:adwords_grouping>\n";
            } else {
            ...
            $arItem['YANDEX_DESCR'] = yandex_text2xml(TruncateText(
                            ($arItem["PREVIEW_TEXT_TYPE"] == "html" ?
                                    strip_tags(preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $arItem["~PREVIEW_TEXT"])) : preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $arItem["~PREVIEW_TEXT"])), 255), true);
                ...
                if (YANDEX_SKU_EXPORT_MIN_PRICE == $arSKUExport['SKU_EXPORT_COND']) {
                    ...
                    while ($obOfferItem = $rsOfferItems->GetNextElement()) {
                        ...
                        if ($XML_DATA['PRICE'] > 0) {
                            ...
                            if ($arPrice = $rsPrices->Fetch()) {
                                if ($arOptimalPrice = CCatalogProduct::GetOptimalPrice(
                                    $arOfferItem['ID'], 1, array(2), 'N', array($arPrice), $arOfferIBlock['LID'], array()
                                )) {
                                    $minPrice = $arOptimalPrice['RESULT_PRICE']['DISCOUNT_PRICE'];
                                    $fullPrice = $arOptimalPrice['RESULT_PRICE']['BASE_PRICE'];
                                    $minPriceCurrency = $arOptimalPrice['RESULT_PRICE']['CURRENCY'];
                            ...
                            if ($arPrice = CCatalogProduct::GetOptimalPrice(
                                $arOfferItem['ID'], 1, array(2), // anonymous
                                'N', array(), $arOfferIBlock['LID'], array()
                            )) {
                                $minPrice = $arPrice['RESULT_PRICE']['DISCOUNT_PRICE'];
                                $fullPrice = $arPrice['RESULT_PRICE']['BASE_PRICE'];
                                $minPriceCurrency = $arPrice['RESULT_PRICE']['CURRENCY'];
                    ...
                    if (!empty($arCurrentOffer) && !empty($arCurrentPrice)) {
                        ...
                        if ($arOfferItem['YANDEX_AVAILABLE']) {
                            $stock = 'in stock';
                        } else {
                            $stock = 'out of stock';
                        }
                        ...
                        $strOfferYandex .= "<g:price>" . $minPrice . " RUB</g:price>\n";
...
//                                        continue;
                                    ...
                                    $strOfferYandex .= "<title>" . $APPLICATION->ConvertCharset( yandex_text2xml($arOfferItem["~NAME"], true), LANG_CHARSET, 'UTF-8') . "</title>\n";
                                    break;
                                case 'description':
                                    $strOfferYandex .= "<description>3";
                                   /* if (strlen($arOfferItem['~PREVIEW_TEXT']) <= 0) {
                                        $mytext = $APPLICATION->ConvertCharset($arItem['YANDEX_DESCR'], LANG_CHARSET, 'UTF-8');
                                         $strOfferYandex.= "тест1_1";
                                    } else {
                                        $mytext = $APPLICATION->ConvertCharset( yandex_text2xml(TruncateText(
                                                        ($arOfferItem["PREVIEW_TEXT_TYPE"] == "html" ?
                                                                strip_tags(preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $arOfferItem["~PREVIEW_TEXT"])) : $arOfferItem["~PREVIEW_TEXT"]), 255), true), LANG_CHARSET, 'UTF-8');
...
//                                    }
                    ...
                    while ($obOfferItem = $rsOfferItems->GetNextElement()) {
                        ...
                        $strOfferYandex.= "<g:availability>" . $stock . "</g:availability>\n";
                        ...
                        foreach ($arYandexFields as $key) {
                            switch ($key) {
                                case 'title':
                                    $strOfferYandex.= "<title>" . gmcConvertText(yandex_text2xml($arItem["~NAME"], true)) . "</title>\n";
                                    break;
                               case 'description':
                                     $strOfferYandex .= "<description>2";
                                   /* if (strlen($arItem['DETAIL_TEXT']) <= 0) {
                                        $strOfferYandex.= gmcConvertText(yandex_text2xml($arItem["~NAME"], true)); //DETAIL_TEXT
                                    } else {
                                         $mytext = gmcConvertText(yandex_text2xml(strip_tags($arItem["DETAIL_TEXT"]), true));
                                         $strOfferYandex.= mb_strimwidth($mytext, 0, 455, "..."); //DETAIL_TEXT
                                    }
* /
                                    $strOfferYandex .= "</description>\n";
                                    break;
...
//                                    }
            ...
            elseif ($arCatalog['CATALOG_TYPE'] == CCatalogSku::TYPE_FULL && $arItem['CATALOG_TYPE'] == Catalog\ProductTable::TYPE_PRODUCT) {
                $str_AVAILABLE = ($arItem['CATALOG_AVAILABLE'] == 'Y' ? true : false);
                ...
                if ($XML_DATA['PRICE'] > 0) {
                    ...
                    if ($arPrice = $rsPrices->Fetch()) {
                        if ($arOptimalPrice = CCatalogProduct::GetOptimalPrice(
                                        $arItem['ID'], 1, array(2), 'N', array($arPrice), $ar_iblock['LID'], array()
                                )) {
                            $minPrice = $arOptimalPrice['RESULT_PRICE']['DISCOUNT_PRICE'];
                            $fullPrice = $arOptimalPrice['RESULT_PRICE']['BASE_PRICE'];
                            $minPriceCurrency = $arOptimalPrice['RESULT_PRICE']['CURRENCY'];
                ...
                $strOfferYandex.= "<g:adwords_redirect>" . $usedProtocol . $ar_iblock['SERVER_NAME'] . htmlspecialcharsbx($arItem["~DETAIL_PAGE_URL"]) . "</g:adwords_redirect>\n"; // . $referer 
                $strOfferYandex.= "<g:condition>new</g:condition>\n";
                if ($str_AVAILABLE) {
                    $stock = 'in stock';
                } else {
                ...
                foreach ($arYandexFields as $key) {
                    ...
                    switch ($key) {
                        case 'title':
                            $title_text = gmcConvertText(yandex_text2xml($arItem["~NAME"], true));
                            $strValue = "<title>" . $title_text . "</title>\n"; // 
                           
                            break;
                        case 'description':
                                    $mytext = gmcConvertText(yandex_text2xml(strip_tags($arItem["DETAIL_TEXT"]), true));
                                    if ($arItem["DETAIL_TEXT"] == "") {
                                        $mytext = gmcConvertText(yandex_text2xml($arItem["~NAME"], true));
                                    }

                                    $strValue = "<description>" . getNiceSubStr($mytext , 355) . "</description>\n";
                            break;
                        case 'param':
                            if ($parametricFieldsExist) {
                                foreach ($parametricFields as $paramKey => $prop_id) {
                        ...
                        case 'g:product_type':
                            ...
                            $arrayBreads = array();
                            $nav = CIBlockSection::GetNavChain($IBLOCK_ID, $arItem['IBLOCK_SECTION_ID']);
                            while ($row = $nav->GetNext()) {
                                $arrayBreads[] = yandex_text2xml($row['NAME'], true);
                            }
                            $strValue = "<".$key.">". gmcConvertText(implode(" &gt; ", $arrayBreads))."</".$key.">\n";
                            break;

                        // С помощью атрибута google_product_category [категория_товара_в_google] можно указать категорию товара по классификации Google.
                        // Если вы будете добавлять эту информацию, ваши объявления будут показаны тем, кто заинтересован в подобных товарах.
                        // https://support.google.com/merchants/answer/6324436?visit_id=1-636240668401198896-1869188554&rd=1
//                        case 'g:google_product_category':
//                            $sectionIds = array(1190, 1191); // Электроника > Связь > Телефония > Мобильные телефоны
//                            if (! in_array($arItem['IBLOCK_SECTION_ID'], $sectionIds)) {
//                                // Получение хлебных крошек
//                                $arrayBreads = array();
//                                $nav = CIBlockSection::GetNavChain($IBLOCK_ID, $arItem['IBLOCK_SECTION_ID']);
//                                while ($row = $nav->GetNext()) {
//                                    $arrayBreads[] = $row['NAME'];
//                                }
//                                $strValue = "<g:google_product_category>". gmcConvertText(implode(" &gt; ", $arrayBreads))."</g:google_product_category>\n";
//                            } else {
//                                $strValue = "<g:google_product_category>267</g:google_product_category>\n";
//                            }
//                            break;
...
//                            }
                ...
                $strOfferYandex .= "<g:google_product_category>2</g:google_product_category>\n";
